  ...
  return (
    <div className="ai-summary">
      ...
      <div className="paper-links">
          {summary.relevantPapers && summary.relevantPapers.length > 0 ? (
          summary.relevantPapers.map((paper, index) => (
          <a key={index} href={paper.url} target="_blank" rel="noopener noreferrer" className="paper-link">
              📄 {paper.title}
              </a>
              ))
              ) : (
              <div className="no-results" role="status" aria-live="polite">
                  ...
                  <div className="subtext">
                      No related papers found for this summary. Currently <strong>{(appData && appData.papers && appData.papers.length) ? appData.papers.length : 0}</strong> publications available out of <strong>608</strong>.
                  </div>
              </div
    )}</div>
    </div>
...
// ExperimentsGrid (show "Coming Soon" when user searched/filtered and no results)
const ExperimentsGrid = ({ experiments = [], categoryFilter = '', missionFilter = '', searchQuery = '', onOpen }) => {
  const maxCount = 608;
  const currentCount = (appData && appData.papers && Array.isArray(appData.papers)) ? appData.papers.length : (experiments.length || 0);

  // determine if user actively searched/filtered
  const hasActiveFilters = !!(
    (searchQuery && String(searchQuery).trim() !== '') ||
    (categoryFilter && String(categoryFilter).trim() !== '') ||
    (missionFilter && String(missionFilter).trim() !== '')
  );

  // filter experiments (customize matching rules if perlu)
  const q = (searchQuery || '').toString().toLowerCase().trim();
  const filteredExperiments = experiments.filter(exp => {
    const categoryMatch = !categoryFilter || (exp.category === categoryFilter);
    const missionMatch = !missionFilter || ((exp.impact || '').toString().toLowerCase().includes(missionFilter.toLowerCase()));
    const searchMatch = !q || (
    ...
    return categoryMatch && missionMatch && searchMatch;
  });

  // Show Coming Soon when user has active filters/search and no result
  if (hasActiveFilters && (!filteredExperiments || filteredExperiments.length === 0)) {
    return (
      <div className="experiments-grid">
        <div className="no-results" role="status" aria-live="polite">
          <div className="title">Coming Soon</div>
          <div className="subtext">
            No results found for your filters/search. Currently <strong>{currentCount}</strong> publications available out of <strong>{maxCount}</strong>. More will be added soon.
          </div>
        </div>
      </div>
    );
  ...
  // normal rendering when results exist
  return (
    <div className="experiments-grid">
      {filteredExperiments.map((experiment) => (
        <ExperimentCard key={experiment.id || experiment._id || experiment.title} experiment={experiment} onOpen={() => onOpen(experiment)} />
      ))}
    </div>
  );
};
...
// FooterNote: statis, selalu tampil di bawah Dashboard
const FooterNote = () => (
  <footer className="site-footer" role="contentinfo" aria-label="Site footer">
    <div className="container footer-inner">
      <p className="dashboard-note">
        Note: This dashboard currently features concise summaries of the most impactful and representative bioscience papers. Additional publications will be integrated in upcoming updates to further expand accessibility and insight.
      </p>
    </div>
  </footer>
);
